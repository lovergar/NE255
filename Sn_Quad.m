...
% note: use atanshift instead of atan in the input
%% 
...
%% Integration
value=0;
biglog=zeros(size(Octants,1)*size(AbsPoints,1),5);
for oct=1:size(Octants,1)
    for i=1:size(AbsPoints,1)
        xyz=AbsPoints(i,:).*Octants(oct,:);
        log(i,1:3)=xyz;
        value=value+fun(xyz(1), xyz(2), xyz(3))*Wspecific(i);
        log(i,4)=fun(xyz(1), xyz(2), xyz(3));
        log(i,5)=fun(xyz(1), xyz(2), xyz(3))*Wspecific(i);
    end
 biglog((oct-1)*size(AbsPoints,1)+1:(oct)*size(AbsPoints,1),:)=log;
end
   out=value;
end
